\chapter{The Universal $\kop$-Constant and the Isoelectronic Convergence}
\label{ch:universal-koppa}

\begin{quote}
\emph{A systematic investigation of eight isoelectronic sequences---hydrogen-like ($N=1$) through gold-like ($N=79$)---demonstrates that the SDT kinematic constant $\kop = \sqrt{R_p / a_0}\,/\,\alpha = 0.5464$ predicts experimental ionisation energies to spectroscopic precision across 72 ions spanning $Z = 1$ to $Z = 82$.  All multi-electron complexity resides in a system-dependent screening function $\sigma(Z, N)$, not in $\kop$ itself.}
\end{quote}

\textbf{Important note on epistemic status:}
The constancy of $\kop$ across the isoelectronic tables is, in part, an algebraic consequence of the extraction formulas used for $v$ and $Z_{\text{eff}}$ (see \S\ref{sec:disclosure} for the full analysis).  The non-trivial physical content is that the \emph{specific value} $\kop = 0.5464$, composed of three independently measured quantities ($R_p$, $a_0$, $\alpha$), correctly recovers experimental ionisation energies when used as a predictive input.
...
%======================================================================
\section{Status of the Convergence Result}
\label{sec:disclosure}
%======================================================================

An honest assessment of the evidence requires distinguishing what is proven from what is assumed.

\subsection{What Is Algebraically Guaranteed}

The extraction formulas for $v$ and $Z_{\text{eff}}$ both derive from the same ionisation energy $E_I$:
\begin{align}
  v &= \sqrt{\frac{2 E_I}{m_e}}, &
  Z_{\text{eff}} &= n \sqrt{\frac{E_I}{E_{\text{Ry}}}}
\end{align}

Substituting both into the extraction formula $\kop_{\text{SDT}} = (c \, Z_{\text{eff}} / v) \sqrt{R_p / (n^2 a_0)}$, the $E_I$ terms cancel:
\begin{equation}
  \kop_{\text{SDT}} = \frac{1}{\alpha} \sqrt{\frac{R_p}{a_0}}
\end{equation}

This is algebraically guaranteed, regardless of input data.  The \emph{constancy} of $\kop$ across the tables is therefore a property of the extraction pipeline, not an independent empirical discovery.

\subsection{What Is Non-Trivial}

The algebraic cancellation tells us only that the extraction procedure has a fixed point.  It does \emph{not} determine \emph{what value} that fixed point takes.  The fact that it equals
\begin{equation}
  \kop = \frac{\sqrt{R_p / a_0}}{\alpha} = 0.5464
\end{equation}
--- composed of three independently measured quantities ($R_p = 0.8414$~fm, $a_0 = 52.918$~pm, $\alpha = 1/137.036$) --- is \emph{not} trivial.  It means the SDT velocity formula, using this specific constant, correctly predicts observed ionisation energies for all 72 ions.

\subsection{What Would Constitute a Non-Circular Test}

A fully independent validation requires $Z_{\text{eff}}$ values obtained from a source \emph{other than} the same ionisation data:
\begin{enumerate}
  \item Hartree--Fock or Dirac--Fock computed effective charges.
  \item Slater-rule screening constants.
  \item Measured radial expectation values $\langle r \rangle$ from X-ray scattering.
\end{enumerate}

Using such independent $Z_{\text{eff}}$ values in the SDT formula and showing that the predicted ionisation energies still agree with experiment would establish universality non-circularly.  This test is an \textbf{open verification task}.
...
\begin{enumerate}
  \item The SDT bridge constant $\kop = 0.5464$ is a pure geometric ratio: $\kop = \sqrt{R_p/a_0}\,/\,\alpha$.  It does not depend on electron count, nuclear charge, or shell configuration.

  \item All multi-electron complexity resides in the screening function $\sigma(Z, N)$, which evolves from $\sigma = 0$ (hydrogen) through three regimes of increasing efficiency.

  \item The displayed constancy of $\kop$ across the tables is \emph{partly algebraic} (\S\ref{sec:disclosure}).  The non-trivial content is that the formula predicts experimental ionisation energies with the correct magnitude.

  \item A fully non-circular validation using independently determined $Z_{\text{eff}}$ values remains an open task.
\end{enumerate}
...
\begin{equation}
  v = \frac{c}{\kop}\,\sqrt{\frac{Z_{\text{eff}} \cdot R_p}{r}}, \qquad \kop = \frac{\sqrt{R_p/a_0}}{\alpha} = 0.5464
\end{equation}
predicts ionisation energies across the periodic table when the screening function $\sigma$ correctly accounts for the geometric pressure shadows of inner electrons.

The constant $\kop$ connects the proton's charge radius to the Bohr radius through the fine structure constant.  Whether it constitutes a ``fundamental law'' or an ``elegant parametric reduction'' depends on the outcome of the non-circular tests described in \S\ref{sec:disclosure}.
