...
The derivation uses only the four SDT axioms:
...
## 7. Calibration to Coulomb's Law

Matching the derived force to Coulomb's law:

$$F = \frac{k_e e^2}{r^2} = \frac{\pi}{4} P_{\text{CMB}} \frac{R_N^2 R_e^2}{r^2} \tag{7.1}$$

Solving for $P_{\text{CMB}}$:

$$\boxed{P_{\text{CMB}} = \frac{4 k_e e^2}{\pi R_N^2 R_e^2}} \tag{7.2}$$

Using:
- $k_e = 8.9875517923 \times 10^9$ N·m²/C²
- $e = 1.602176634 \times 10^{-19}$ C
- $R_N = 8.4 \times 10^{-16}$ m
- $R_e = 10^{-21}$ m

$$P_{\text{CMB}} = \frac{4 \times 8.988 \times 10^9 \times (1.602 \times 10^{-19})^2}{\pi \times (8.4 \times 10^{-16})^2 \times (10^{-21})^2}$$

$$P_{\text{CMB}} = 4.16 \times 10^{44} \text{ Pa}$$

### 7.1 Pressure Scaling Law

The relationship between exclusion radius and CMB pressure is geometric and mandatory:

$$P_{\text{CMB}} \propto \frac{1}{R_e^2} \tag{7.3}$$
...
At the Planck scale ($R_e \sim 10^{-35}$ m), the pressure reaches extreme values ($\sim 10^{72}$ Pa), reflecting that at the smallest possible exclusion radius, the entire universe's weight is concentrated across the minimal cross-section.
...
- CMB pressure: $P_{\text{CMB}} = 4.16 \times 10^{44}$ Pa (for $R_e = 10^{-21}$ m)
...
**Benchmark:** B01 (Coulomb Force from CMB Occlusion)

---

**Cross-Reference:** See Phase 15 for the derivation of gravitation from the same CMB pressure field.
